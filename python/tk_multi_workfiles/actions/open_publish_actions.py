...
from .open_file_action import (
    OpenFileAction,
    CopyAndOpenInCurrentWorkAreaAction,
    ContinueFromFileAction,
)
...
class ContinueFromPublishAction(ContinueFromFileAction):
    ...
    def execute(self, parent_ui):
        ...
        return self._continue_from(
            src_path, self.environment.publish_template, parent_ui
        )
...
class CopyAndOpenPublishInCurrentWorkAreaAction(CopyAndOpenInCurrentWorkAreaAction):
    ...
    def execute(self, parent_ui):
        ...
        return self._open_in_current_work_area(
            self.file.publish_path,
            self.environment.publish_template,
            self.file,
            self.environment,
            parent_ui,
        )
