...
class OpenOptionsForm(QtGui.QWidget):
    ...
    def __init__(
        ...
        self._ui.publish_frame.setStyleSheet(
            ss % ("publish_frame", "publish_frame", "publish_frame")
        )
        self._ui.publish_ro_frame.setStyleSheet(
            ss % ("publish_ro_frame", "publish_ro_frame", "publish_ro_frame")
        )
        self._ui.work_file_frame.setStyleSheet(
            ss % ("work_file_frame", "work_file_frame", "work_file_frame")
        )
        ...
        self._ui.publish_ro_frame.mousePressEvent = (
            self._on_publish_ro_mouse_press_event
        )
    ...
    def _update_details(self):
        ...
        if self._work_file:
            ...
            if self._work_file.thumbnail:
                self._set_thumbnail(
                    self._ui.work_file_thumbnail, self._work_file.thumbnail
                )
            elif self._work_file.thumbnail_path:
                thumbnail_path = self._download_thumbnail(
                    self._work_file.thumbnail_path
                )
                if thumbnail_path:
                    self._set_thumbnail(
                        self._ui.work_file_thumbnail, QtGui.QPixmap(thumbnail_path)
                    )
        ...
        if self._publish_requires_copy:
            self._ui.publish_note.setText(
                "<small>(Note: The published file will be copied to "
                "your work area as version v%03d and then opened)</small>"
                % self._next_version
            )
        else:
            self._ui.publish_note.setText("")
        ...
        try:
            ...
            if self._mode == OpenOptionsForm.OPEN_PUBLISH_MODE:
                ...
                QtGui.QWidget.setTabOrder(
                    self._ui.work_file_frame, self._ui.publish_frame
                )
                QtGui.QWidget.setTabOrder(
                    self._ui.publish_frame, self._ui.publish_ro_frame
                )
                ...
                QtGui.QWidget.setTabOrder(
                    self._ui.publish_frame, self._ui.work_file_frame
                )
                QtGui.QWidget.setTabOrder(
                    self._ui.work_file_frame, self._ui.publish_ro_frame
                )
    ...
    def _download_thumbnail(self, url):
        ...
        """
        try:
            path_to_cached_thumb = shotgun_data.ShotgunDataRetriever.download_thumbnail(
                url, self._app
            )
        except Exception as e:
            self._app.log_debug(
                "Could not get thumbnail for url '%s'. Error: %s" % (url, e)
            )
